...
const ROLES: {
...
}[] = [
  {
    ...
    icon: GraduationCap,
    to: "/student",
    gradient: "linear-gradient(135deg, #FF5A6A, #8B5CF6)",
    glow: "0 0 40px rgba(255,90,106,0.3)",
  },
  {
    id: "employer",
    title: "Organization",
    ...
    glow: "0 0 40px rgba(34,211,153,0.3)",
  },
  {
    id: "gov",
    title: "Government",
  ...
  return (
    <div className="min-h-screen" style={{ background: "#060812" }}>
      ...
      <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        <Logo />
        <span className="text-[12px] text-slate-500">Concept · v2</span>
      </header>
      ...
      >
        <h1 className="text-balance font-display text-[52px] font-bold leading-[1.02] tracking-[-0.03em] text-white md:text-[80px]">
          Close the distance
          <br />
          between <span className="neon-text">skills</span> and{" "}
          <span className="neon-text">opportunity</span>.
        </h1>
          ...
          {ROLES.map((r) => {
            ...
            return (
              <motion.button
                ...
                type="button"
                onClick={() => { setRole(r.id); void navigate({ to: r.to }); }}
              ...
              >
                <span className="grid h-12 w-12 place-items-center rounded-2xl"
                  style={{ background: r.gradient, boxShadow: r.glow }}>
                  <Icon className="h-5 w-5 text-white" />
                </span>
                <h3 className="mt-5 font-display text-[24px] font-bold leading-tight text-white">
                  {r.title}
                </h3>
                ...
                <ul className="mt-4 space-y-1.5">
                  {r.bullets.map((b) => (
                    <li key={b} className="flex items-center gap-2 text-[13px] text-slate-500">
                      <span className="h-1 w-1 rounded-full" style={{ background: r.gradient.includes("FF5A6A") ? "#FF5A6A" : r.gradient.includes("34D399") ? "#34D399" : "#8B5CF6" }} />
                      {b}
                    </li>
                  ))}
                </ul>
                <div className="mt-6 inline-flex items-center gap-2 self-start rounded-full px-5 py-2.5 text-[13px] font-semibold text-white transition-all"
